...
Para cada fila de la hoja "Altas" (Nombre en A, Costo en F, Meses en G) busca la
respuesta correspondiente en el Google Form y completa:

    C  Correo         -> respuesta a "CORREO DEL ALUMNO" (se trae del formulario)
...
El emparejamiento es por NOMBRE (columna A) con similitud difusa (tolera acentos
y el orden apellido/nombre), comparando contra "NOMBRE COMPLETO DEL ALUMNO" y
"NOMBRE DEL PADRE O TUTOR". Solo acepta si supera el umbral; si varias personas
DISTINTAS tienen un nombre igual de parecido (correos distintos), es ambiguo y
no se adivina.
...
# El emparejamiento fila↔formulario es por NOMBRE (columna A). Umbral mínimo de
# similitud (0-100) para aceptar; y margen para considerar "empate" al detectar
# nombres ambiguos (varias personas distintas con nombre igual de parecido).
NAME_MATCH_THRESHOLD = int(os.environ.get("NAME_MATCH_THRESHOLD", "88"))
NAME_MATCH_MARGIN = int(os.environ.get("NAME_MATCH_MARGIN", "6"))
...
# Emparejamiento                                                               #
# --------------------------------------------------------------------------- #
def best_match(name: str, records: list[dict]) -> tuple[dict | None, float, str]:
    """Empareja por NOMBRE (columna A) contra el formulario.

    Devuelve (respuesta, score, error). Solo acepta si la similitud supera el
    umbral. Si varias personas DISTINTAS (correos distintos) tienen un nombre
    igual de parecido, es ambiguo y NO se adivina (deja error). Si son varias
    respuestas de la MISMA persona (mismo correo), toma la más reciente.
    """
    if not name.strip():
        return None, 0.0, "sin nombre en la fila"
    ...
    scored = sorted(((score(r), r) for r in records), key=lambda x: x[0], reverse=True)
    if not scored or scored[0][0] < NAME_MATCH_THRESHOLD:
        return None, (scored[0][0] if scored else 0.0), "sin coincidencia de nombre"

    top = scored[0][0]
    close = [r for s, r in scored if s >= top - NAME_MATCH_MARGIN]
    emails = {r["email"] for r in close if r["email"]}
    if len(emails) > 1:
        return None, top, "nombre ambiguo (varias personas con ese nombre)"

    best = max(close, key=lambda r: r["create_time"])
    return best, top, ""
...
def main() -> None:
    ...
    for i, row in enumerate(data_rows):
        ...
        nombre = cell(row, 0)     # A
        correo_c = cell(row, 2)   # C (lo que ya haya escrito, puede estar vacío)
        costo = cell(row, 5)      # F
        meses = cell(row, 6)      # G

        if not nombre and not correo_c:
            continue  # fila vacía
        ...
        # C, D y E: dependen del emparejamiento con el formulario POR NOMBRE.
        especialidad = ""
        curso = ""
        correo_form = ""
        match, score, err = best_match(nombre, records)
        if match:
            n_matched += 1
            especialidad = match["carrera"].strip()
            curso = first_word(match["curso_values"][0]) if match["curso_values"] else ""
            correo_form = match["email_raw"].strip()
            print(
                f"  Fila {sheet_row}: «{nombre}» -> «{match['name_student'] or match['name_tutor']}»"
                f" (sim {score:.0f}) | {correo_form} | {especialidad} | {curso}"
            )
        else:
            n_not_found += 1
            print(f"  Fila {sheet_row}: «{nombre}» -> SIN COINCIDENCIA ({err})")

        # Correo destinatario: el de C si ya existe, si no el traído del form.
        correo = correo_c or correo_form
        ...
        schedule("C", 2, correo_form)  # trae el correo del formulario a la col. C
